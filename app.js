...
function reqPost (request, response) {
    if (request.method == 'POST') {
        ...
        request.on('end', function () {
			...
			var sInsert = `INSERT INTO private_individuals (first_name, second_name, patronymic, inn, passport, snils, driver_license, additional_docs, notes)
            ...
            console.log('Вставка завершена!: '+sInsert);
        });
    }
}
...
function ViewSelect(res) {
	...
	for(let i=0; i < results.length; i++) 
		res.write('<tr><td>'
            ...
            + results[i].snils
            +'</td><td>'
            + results[i].driver_license
            +'</td><td>'
            + results[i].additional_docs
            +'</td><td>'
            + results[i].notes
            +'</td></tr>'
        );
